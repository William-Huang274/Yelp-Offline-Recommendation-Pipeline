...
这是一个面向 Yelp 路易斯安那州餐饮推荐场景的离线推荐与重排项目。当前版本围绕三层主线展开：

- 召回路由（`Stage09`）
- 结构化精排（`Stage10`）
- 基于奖励模型的救援式重排（`Stage11`）

其中，`Stage11` 使用同一个 `Qwen3.5-9B` 奖励模型主干，并在此基础上拆成三个按排名窗口分工的专家模型：
...
当前 README 主展示的 `Stage11` 结果，统一基于中高交互用户集（`bucket5`）。这也是当前最适合作为对外展示口径的一条主线。
...
当前公开结果使用的是同一套 source-parity 业务范围。过滤后覆盖：
...
`Stage11` 不是三个互不相关的大模型，而是同一个 `Qwen3.5-9B` 奖励模型主干下的三个专家模型，分别负责 `11-30`、`31-60`、`61-100` 三个候选区间。
...
仓库另外保留了一份“上一版 vs 当前版”的内部对照文档，用来说明：

- 上一版冻结了什么
- 当前版本为什么能在三层主线上拿到更大提升
- `Stage11` 为什么从通用 `SFT / DPO` 路线切换到奖励模型救援重排

本地入口：

- `docs/release/version_comparison_previous_vs_current_20260410.zh-CN.md`

这份文档只作为本地 release 对照材料保留，不纳入当前公开技术说明表面。
