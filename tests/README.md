...
- `test_full_chain_smoke.py`: runs the safe `stage01 -> stage11` smoke path, including stage01-08 help entrypoints, the Stage01-11 mini demo, local stage09/10 checks, serving checks, and Stage11 demo / prompt checks
- `test_demo_tools.py`: checks serving config, strategy switching, fallback counters, mock API health, and the Stage01-11 mini demo contract
