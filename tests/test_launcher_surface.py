...
def test_runtime_shell_launchers_exist_for_wrapper_targets(repo_root: Path) -> None:
    expected = [
        repo_root / "scripts" / "runtime_sh" / "run_stage09_bucket5_structural_v5_sourceparity.sh",
        repo_root / "scripts" / "runtime_sh" / "run_stage10_bucket5_structural_v5_joint_min_cls_typed_intent_phase3_slicefix.sh",
        repo_root / "scripts" / "runtime_sh" / "run_stage11_bucket5_top250_semantic_compact_rm_train.sh",
        repo_root / "scripts" / "runtime_sh" / "run_stage11_bucket5_top250_semantic_compact_rm_eval_only.sh",
    ]
    missing = [path for path in expected if not path.exists()]
    assert not missing, f"missing runtime shell launchers: {missing}"
