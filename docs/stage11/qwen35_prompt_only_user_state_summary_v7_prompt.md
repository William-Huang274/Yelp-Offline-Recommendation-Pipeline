# Qwen3.5-35B Prompt-Only User State Summary V7 Template
...
- 3 refs is preferred upper bound.
- 4 refs is allowed only when the claim is still narrow and concrete.
- Do not cite long runs like `event_1` through `event_8`.
- If a claim would require more than 4 refs, narrow the claim, demote it to a hypothesis, or move it to `unknowns`.
...
- `grounded_facts` and `discriminative_signals` should usually stand on 1 to 3 refs.
...
- Avoid "preference bundles" that join multiple cuisines, scenes, or properties unless each component is directly supported by the same small evidence subset.
...
- A specific supported service sensitivity claim is enough to resolve `service_tolerance`; use `tolerance_threshold` only when the unresolved part materially affects how the user should be modeled.
