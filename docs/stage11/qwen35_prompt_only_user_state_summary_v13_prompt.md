# Qwen3.5-35B Prompt-Only User State Summary V13 Template
...
Step 4. Run the minimal validity check before emitting JSON.
- Max list sizes are upper bounds, not targets. Leave a list shorter if you do not have distinct high-signal items.
- Do not repeat the same or nearly identical claim within the same section.
- Unknown field names must be chosen only from:
  - `cuisine_preference`
  - `scene_preference`
  - `tolerance_threshold`
  - `recent_shift`
- If `stable_preferences` or `discriminative_signals` already contain a direct supported cuisine preference, do not also emit `cuisine_preference` in `unknowns`.
