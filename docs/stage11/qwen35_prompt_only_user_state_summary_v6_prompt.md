# Qwen3.5-35B Prompt-Only User State Summary V6 Template
...
- Prefer 1 to 3 atomic refs per claim.
- 4 refs is the hard upper bound and should be rare.
...
- `discriminative_signals` should usually cite the single strongest supporting evidence subset, not the union of all related events.
...
- A specific supported service sensitivity claim is enough to resolve `service_tolerance`; use `tolerance_threshold` or a hypothesis if the unresolved part is only the boundary condition.
- Use `recent_shift` unknown only when the evidence cannot distinguish between a real shift and a one-off fluctuation.
