# Qwen3.5-35B Prompt-Only User State Summary V12 Template
...
Step 4. Run the final validity check before emitting JSON.
- Max list sizes are upper bounds, not targets. Do not fill slots just because space is available.
- Never repeat or near-repeat the same claim, reason, or signal across or within sections.
- If two items say almost the same thing, keep only the stronger one.
- Unknown field names must be chosen from this exact set only:
  - `cuisine_preference`
  - `scene_preference`
  - `tolerance_threshold`
  - `recent_shift`
- Do not emit `service_tolerance` or any other unknown field name.
- If you have a direct supported cuisine preference in `stable_preferences` or `discriminative_signals`, then `cuisine_preference` is resolved. Do not also emit `cuisine_preference` unknown.
- If you have a direct supported one-sided service sensitivity claim, do not emit `tolerance_threshold` unknown unless there is also direct evidence of tolerated outcomes and the unresolved boundary matters.
- Every `evidence_refs` list must satisfy the ref limit before you return the JSON.
- If any section would contain duplicates, generic filler, or invalid ref counts, shorten the section instead of padding it.
...
- A specific supported service sensitivity claim is enough to resolve service tolerance at the user-state level.
...
- refs: `rev_8`, `tip_1`, `tip_2`, `tip_3`, `tip_4`
Why bad:
- Too many refs. The claim is valid only after shrinking the evidence set.
Better:
- `stable_preferences`: "Strong preference for oysters and seafood dishes."
- refs: `rev_8`, `tip_1`

Example 4. Unknown overuse.
Bad:
- `stable_preferences`: "Strong preference for oysters."
- `unknowns`: `cuisine_preference` because "the full cuisine range is broader or context-dependent"
Why bad:
- A direct supported cuisine preference already resolves `cuisine_preference`.
Better:
- Keep the supported cuisine claim and omit `cuisine_preference` unknown.

Example 5. Duplicate padding.
Bad:
- three repeated `stable_preferences` entries with the same oyster claim
- two repeated `context_rules` entries with the same city-concentration claim
Why bad:
- Max counts are not targets and repeated entries add no information.
Better:
- keep one unique oyster claim
- keep one unique city context rule
...
- pad lists with duplicate items
