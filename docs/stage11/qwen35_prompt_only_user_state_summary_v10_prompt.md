# Qwen3.5-35B Prompt-Only User State Summary V10 Template
...
- Do not convert merchant-side tags such as `family_friendly`, `full_bar`, `good_for_kids`, `outdoor_seating`, `table_service`, `nightlife`, `casual_vibe`, or similar tags into user `stable_preferences` or `discriminative_signals` unless there is direct user-authored text evidence or repeated cross-merchant evidence that clearly supports the user-level finding.
- If a claim relies only on `event_*` refs and merchant-side tags, phrase it as observed behavioral context in `recent_signals` or `context_rules`, not as a stable preference.
- Do not infer latent traits such as "tourist", "visitor", "family-oriented", or "bar-seeking" from city concentration, merchant properties, or broad sequence patterns alone.
- A user-level property or scene preference must be supported by direct textual evidence, explicit praise or complaints, or a tight repeated pattern across different merchants with a small evidence set.
- When in doubt, keep merchant attributes at the merchant-context level or move them to `unknowns`; do not promote them into a high-confidence user trait.
