...
| 接口 | 命令 | 覆盖内容 | 本地验证 |
| --- | --- | --- | --- |
| Release check | `python tools/release/run_release_checks.py --skip-pytest` | public surface、current release、smoke CLI | PASS |
| Contract smoke | `python tools/demo/run_stage01_11_minidemo.py` | Stage01-to-Stage11 最小内存链路 | PASS |
| Release summary | `python tools/demo/demo_recommend.py summary` | Stage09/10/11 冻结摘要 | PASS |
| Case replay | `python tools/demo/demo_recommend.py show-case --case boundary_11_30` | Stage11 case replay | PASS |
| Batch inference | `python tools/serving/batch_infer_demo.py --request-id stage11_b5_u001072 --strategy reward_rerank --stage09-mode lookup_live --stage10-mode xgb_live --stage11-mode replay` | request-level 推理路径 | PASS |
| Cache miss inference | `python tools/serving/batch_infer_demo.py --request-id stage11_b5_u006562 --strategy reward_rerank --stage09-mode lookup_live --stage10-mode xgb_live --stage11-mode replay --simulate-stage11-cache-miss` | Stage11 cache miss 与 Stage10 fallback | PASS |
| HTTP self-test | `python tools/serving/mock_serving_api.py --self-test` | `/health` 和内置 `/rank` 样例 | PASS |
| HTTP direct POST | `Invoke-RestMethod ... -Body (Get-Content config/demo/online_rank_minimal_input.json -Raw)` | 直接调用 `/rank` 最小请求 | PASS |
| HTTP online test | `python tools/serving/load_test_mock_serving.py --url http://127.0.0.1:18081/rank ...` | 本地 HTTP `/rank`、mixed traffic、fallback | PASS |
| Engineering load runner | `python tools/serving/run_serving_engineering_load.py --duration-s 5 ...` | 持续压测脚本、JSON/Markdown 导出路径 | PASS |
