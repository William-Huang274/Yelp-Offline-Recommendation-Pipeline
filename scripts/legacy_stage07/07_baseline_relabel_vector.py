...
CITY_TOPK = 8
RANDOM_SEED = 42
BACKFILL_MODEL_NAME = "sentence-transformers/all-MiniLM-L6-v2"
ENABLE_BACKFILL_MISSING_EMBEDDINGS = False
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--enable-backfill-missing-embeddings",
        action="store_true",
        default=ENABLE_BACKFILL_MISSING_EMBEDDINGS,
        help="Try sentence-transformers to backfill rows missing from cache.",
    )
...
def norm_label_token(x: object) -> str:
    if x is None:
        return ""
    try:
        if pd.isna(x):
...
def recalc_one(row: pd.Series) -> tuple[str, int, str, str]:
    l1 = norm_label_token(row.get("l1_label", ""))
    top1 = norm_label_token(row.get("l2_label_top1", ""))
    top2 = norm_label_token(row.get("l2_label_top2", ""))
    ...
    cluster_l2 = norm_label_token(row.get("cluster_dominant_l2", ""))
...
def add_recalc_labels(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["label_changed"] = out["l2_label_recalc"].map(norm_label_token) != out["l2_label_top1"].map(norm_label_token)
    return out
...
def stratified_baseline_sample(df: pd.DataFrame, baseline_size: int) -> pd.DataFrame:
    ...
    # Keep general-label cases prioritized so they are not consumed by low-conf overlap first.
...
def backfill_missing_embeddings(texts: list[str]) -> np.ndarray:
    os.environ.setdefault("USE_TF", "0")
    os.environ.setdefault("TRANSFORMERS_NO_TF", "1")
    from sentence_transformers import SentenceTransformer

    model = SentenceTransformer(BACKFILL_MODEL_NAME)
    embs = model.encode(
        texts,
        batch_size=32,
...
def supplement_from_compatible_caches(
    cache_map: dict[str, np.ndarray],
    missing_ids: set[str],
    primary_cache: Path,
    expected_dim: int,
) -> int:
    cache_dir = primary_cache.parent
    extras = [p for p in cache_dir.glob("*.npz") if p.resolve() != primary_cache.resolve()]
    extras.sort(key=lambda p: p.stat().st_mtime, reverse=True)
    added = 0
    target = set(missing_ids)
    for p in extras:
        if not target:
            break
        try:
            d = np.load(p, allow_pickle=False)
            embs = np.asarray(d["embeddings"], dtype=np.float32)
            if embs.ndim != 2 or embs.shape[1] != expected_dim:
                continue
            ids = d["business_id"].astype(str).tolist()
            for i, bid in enumerate(ids):
                if bid in target:
                    cache_map[bid] = embs[i]
                    target.remove(bid)
                    added += 1
            if added > 0:
                print(f"[INFO] supplemented {added} embeddings from cache: {p.name}")
        except Exception:
            continue
    return added
...
def create_vectors(
    ...
    enable_backfill_missing_embeddings: bool = ENABLE_BACKFILL_MISSING_EMBEDDINGS,
    ...
    cache_map = {
        bid: emb_mat[i]
        for i, bid in enumerate(emb_ids.astype(str).tolist())
    }
    missing = work[~work["business_id"].isin(cache_map)].copy()
    supplemented_count = 0
    if not missing.empty:
        supplemented_count = supplement_from_compatible_caches(
            cache_map=cache_map,
            missing_ids=set(missing["business_id"].astype(str).tolist()),
            primary_cache=embedding_cache,
            expected_dim=int(emb_mat.shape[1]),
        )
        missing = work[~work["business_id"].isin(cache_map)].copy()

    backfilled_count = 0
    if enable_backfill_missing_embeddings and not missing.empty:
        try:
            miss_texts = missing.apply(build_embedding_text, axis=1).tolist()
            miss_embs = backfill_missing_embeddings(miss_texts)
            if miss_embs.shape[1] == emb_mat.shape[1]:
                for bid, vec in zip(missing["business_id"].astype(str).tolist(), miss_embs):
                    cache_map[bid] = vec.astype(np.float32)
                backfilled_count = int(len(missing))
            else:
                print(
                    "[WARN] backfill embedding dim mismatch: "
                    f"cache={emb_mat.shape[1]}, backfill={miss_embs.shape[1]}; skip backfill"
                )
        except Exception as exc:
            print(f"[WARN] failed to backfill missing embeddings: {exc}")

    work = work[work["business_id"].isin(cache_map)].copy().reset_index(drop=True)
    semantic = np.stack(
        [cache_map[bid] for bid in work["business_id"].astype(str).tolist()],
        axis=0,
    ).astype(np.float32)
    ...
    meta = {
        ...
        "n_missing_in_cache": int(len(missing)),
        "n_supplemented_from_cache": int(supplemented_count),
        "n_backfilled": int(backfilled_count),
    }
...
def main() -> None:
    ...
    np.savez(
        vectors_npz,
        business_id=np.asarray(vector_df["business_id"].astype(str).to_list(), dtype="<U64"),
        ...
        [
            {
                ...
                "n_missing_in_cache": int(vec_meta["n_missing_in_cache"]),
                "n_supplemented_from_cache": int(vec_meta["n_supplemented_from_cache"]),
                "n_backfilled": int(vec_meta["n_backfilled"]),
